...
In almost all cases, a smaller data size results in a higher relative overhead. This is because this reduces the time spent in code that is common to both C and AOT versions. Taking \mybench{bubble sort} as an example, both versions do the same number of loads, comparisons, and array stores. When operating on 16-bit data, this only takes half as long as for 32-bit data, thus making the part of the overhead that is not affected a more significant factor in the total performance.

For example, \mybench{bubble sort}'s overhead primarily comes from calculating array element locations (see Section \ref{sec-evaluation-bubble-sort}). For a 32-bit array, this takes 17 cycles, 8 of which are spent on the actual load or store, but for 16-bit array access this is only 4 out of 13 cycles.
