...
\tcheck{chk-brtarget-exists}
	& Branch instructions branch to an index < the number of BRTARGETs announced in the method header. \\

\tcheck{chk-all-brtargets-found}
	& At the end of each method, we have seen the number of BRTARGET instructions announced in the method header. \\
...
\tcheck{chk-stack-is-empty-after-return}
	& After popping the return value the stack should be empty. \\

\tcheck{chk-sufficient-stack-space-at-invokelight}
	& At the point of an INVOKELIGHT instruction, the max stack of the caller >= the current stack depth - the number of arguments to the callee + the max stack of the callee. \\
