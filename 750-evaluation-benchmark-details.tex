...
After the reference signature is loaded, the fixed offsets in the C structure means using the signature is also more efficient in C than in Java, which must again follow a number of references to reach the data. We discuss this in more detail in Section \ref{sec-nested-data}.
