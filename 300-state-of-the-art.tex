...
Their approach is to translate JVM bytecode in a single pass, as it is being received by the node, and write native code to flash memory instead of the JVM bytecode. The translation process is very straightforward: each JVM bytecode instruction is mapped to a fixed sequence of native instructions that implement it.


They use the native st
