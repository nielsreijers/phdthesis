...
In this dissertation we propose CapeVM, a sensor node JVM aimed at delivering both high performance and a sandboxed execution environment that guarantees malicious code cannot corrupt the VM's internal state or perform actions not allowed by the VM.
...
Using a set of 12 benchmarks with varying characteristic, including the standard \mybench{CoreMark} benchmark, and a number of real sensor node applications, we show this results in an average performance roughly 2.1x slower than unsafe native code. Without safety checks, this drops to 1.7x. Thus, CapeVM combines the desirable properties of existing work on both safety and virtual machines for sensor networks, with performance overhead than existing solutions.
\bigbreak
