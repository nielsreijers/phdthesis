...
\begin{figure*}[]
  ...
  \caption{CoreMark method calls vs duration}
  \label{fig-coremark-method-calls-vs-duration}
\end{figure*}


% case                  & call method   & size overhead    & perf overhead
% slow large methods    & normal invoke & none             & high
% smallest leaf methods & inlining      & code duplication (serious when used for all but the smallest methods) & none
% in-between            & lightweight   & none             & low


Lightweight methods give us third choice, in between a normal method call and method inlining. When calling a lightweight method, we directly \mycode{CALL} the method's code. We bypass the VM completely, reusing the caller's stack frame, and leaving the parameters on the (caller's) stack. In effect, the lightweight method behaves similar to inlined code, but since we can \mycode{CALL} it from multiple places, we do not incur the code size overhead of inlining large methods.
