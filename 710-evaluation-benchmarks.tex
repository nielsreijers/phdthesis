...
A set of twelve different benchmarks is used to measure the effect of the optimisations, shown in Table \ref{tbl-benchmarks}. This mix of benchmarks was chosen for several reasons. A number of benchmarks, \mybench{bubble sort}, \mybench{binary search}, \mybench{MD5}, \mybench{FFT}, and \mybench{Outlier detection} were chosen because they are used in various related work, enabling us to compare CapeVM to their results.
...
The \mybench{CoreMark} benchmark is an industry standard benchmark to measure CPU performance. It mixes several kinds of processing: linked list processing, a state machine, and matrix operations.
...
Finally, the \mybench{Outlier detection}, \mybench{LEC}, \mybench{MoteTrack} and \mybench{heat detection} benchmarks are all code that was specifically developed for sensor nodes, and \mybench{FFT} is a typical signal processing operation, which is a common task for sensor nodes.
