...
A set of twelve different benchmarks, shown in Table \ref{tbl-benchmarks}, is used to measure the effect of the optimisations and the overhead of safety checks. Table \ref{tbl-evaluation-benchmark-characteristics} shows some key characteristics of these benchmarks: their code size, stack depth, and mix of executed instructions. This mix of benchmarks was chosen for several reasons. A number of benchmarks, \mybench{bubble sort}, \mybench{binary search}, \mybench{MD5}, \mybench{FFT}, and \mybench{Outlier detection} were chosen because they are used in various related work, allowing a comparison of CapeVM to these results.
